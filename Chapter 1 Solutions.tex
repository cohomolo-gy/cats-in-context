...
The components $\alpha_d$ must translate $(F dom) d \nt (G cod) d$ for $d = dom c$. Because $c \in F \downarrow G$, it is a triple,  $(d, e, f : Fd \to Ge)$, $(Fdom) c$ is $Fd$ for $d \in \dd$. In order to translate $Fd \to Gd$, we must factor through $F \downarrow G$. Lets do this one in meetup.
